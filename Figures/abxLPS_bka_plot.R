...
df2 <- df2 %>% 
  mutate(timenum = str_replace(time, "jan24bka", "0")) %>%
  mutate(timenum = str_replace(timenum, "jan26bka", "2")) %>%
  mutate(timenum = str_replace(timenum, "feb1bka", "8")) %>%
  mutate(timenum = str_replace(timenum, "feb8bka", "13"))
...
df2 <- df2 %>%
  mutate(positionName = str_replace(time, "jan24bka", "Post-Antibiotics")) %>%
  mutate(positionName = str_replace(positionName, "jan26bka", "24hr Post-injection")) %>%
  mutate(positionName = str_replace(positionName, "feb1bka", "1week Post-injection")) %>%
  mutate(positionName = str_replace(positionName, "feb8bka", "2week Post-injection"))  
...
ggplot(data = df2, aes(x = timenum, y = mean, group = Treatments)) +
  ...
  annotate("text", x = c(3), y=75, colour= "purple", label = c("LPS\nChallenge"))+
