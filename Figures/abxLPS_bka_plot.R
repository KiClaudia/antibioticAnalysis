...
df2 <- df2 %>% 
  ...
  mutate(timenum = str_replace(timenum, "feb1bka", "5")) %>%
  mutate(timenum = str_replace(timenum, "feb8bka", "7"))
...
ggplot(data = df2, aes(x = factor(time, level=c('jan24bka', 'jan26bka', 'feb1bka', 'feb8bka')), y = mean, group = Treatments)) +
  ...
  scale_x_discrete(breaks=c('jan24bka', 'jan26bka', 'feb1bka', 'feb8bka'), 
