...
df <- gi %>%
  ...
  gather(key = "time", value = "BKA",jan24totri, feb8totri,) %>%
...
ggplot(data = df2, aes(x = factor(time, level=c('jan24totri', 'feb8totri')), y = mean, group = Treatments)) +
  ...
  scale_y_continuous(limits = c(0,5), name = "Total Triglycerides (mg/mL)") +
  scale_x_discrete(breaks=c('jan24totri', 'feb8totri'), 
